...
def voxel(path, edge_length):
    ...
    EDGE_LENGTHS = np.array([edge_length,edge_length,edge_length])
    ...
    def form_surfaces(faces, vertices):
        ...
        # print(faces, ">>>>")
        for face in faces:
            v1_v2 = form_edge(vertices[face[1] - 1], vertices[face[0] - 1])
            v1_v3 = form_edge(vertices[face[0] - 1], vertices[face[2] - 1])
            v2_v3 = form_edge(vertices[face[1] - 1], vertices[face[2] - 1])
    ...
    #Todo: find a more reliable function than average
...
cube_space = voxel("tinker.obj", edge_length)

from vpython import sphere, vec, color, box, vector

box(pos=vec(0,0,0), color=color.green, opacity=1)
box(pos=vec(edge_length,0,0), color=color.red, opacity=1)
box(pos=vec(0,edge_length,0), color=color.blue, opacity=1)
box(pos=vec(0,0,edge_length), color=color.yellow, opacity=1)
box(pos=vec(edge_length,edge_length,0), color=color.red, opacity=1)
box(pos=vec(0,edge_length,edge_length), color=color.red, opacity=1)
box(pos=vec(edge_length,0,edge_length), color=color.red, opacity=1)
box(pos=vec(edge_length,edge_length,edge_length), color=color.red, opacity=1)
